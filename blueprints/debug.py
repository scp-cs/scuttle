...
from flask import Blueprint, redirect, url_for, current_app, request, render_template, send_from_directory, flash, abort, send_file, session
...
@DebugToolsController.route('/admin/settings/api')
@login_required
def api_settings():
    newkey = session.get("API_GENERATED_KEY", None)
    if newkey is not None:
        del session['API_GENERATED_KEY']
    keys = list(ApiKey.select().execute())
    for k in keys:
        k.key = k.key[:16] + '*'*16
    return render_template("debug/apikeys.j2", keys=keys, newkey=newkey)

@DebugToolsController.post('/admin/settings/api/newkey')
@login_required
def new_api_key():
    ...
    session['API_GENERATED_KEY'] = key
    info(f"New API key generated by {current_user.nickname} (ID: {current_user.get_id()})")
    return redirect(url_for("DebugToolsController.api_settings"))

@DebugToolsController.post('/admin/settings/api/deactivate')
@login_required
def remove_api_key():
